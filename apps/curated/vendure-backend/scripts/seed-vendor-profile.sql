-- Seed Vendor Profile for Test Account
-- Creates vendor profile, statistics, and quality metrics for vendor@jade-marketplace.test

\echo 'Creating vendor profile for test account...'
...
BEGIN
    ...
    IF v_user_id IS NULL THEN
        RAISE EXCEPTION 'Vendor user not found. Please run seed script first.';
    END IF;

    RAISE NOTICE 'Found vendor user: %', v_user_id;
    ...
    -- Get profile ID
    SELECT id INTO v_profile_id FROM jade.vendor_profile WHERE vendure_seller_id = v_user_id;
