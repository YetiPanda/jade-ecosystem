/**
 * Product Taxonomy Schema - Direct SQL Implementation
 *
 * Week 3 Day 1: Advanced Product Taxonomy
 * Run this manually with: psql -d jade_marketplace -f sql/003-product-taxonomy.sql
 */
...
CREATE INDEX idx_product_category_parent ON jade.product_category(parent_id);
CREATE INDEX idx_product_category_level ON jade.product_category(level);
CREATE INDEX idx_product_category_slug ON jade.product_category(seo_slug);

-- Product Functions Table
...
-- Product Taxonomy Junction Table
CREATE TABLE jade.product_taxonomy (
  ...
  reviewed_by UUID, -- TODO: Add FK to jade.user when user table exists
...
CREATE INDEX idx_product_taxonomy_product ON jade.product_taxonomy(product_id);
CREATE INDEX idx_product_taxonomy_category ON jade.product_taxonomy(category_id);
CREATE INDEX idx_product_taxonomy_format ON jade.product_taxonomy(product_format_id);
CREATE INDEX idx_product_taxonomy_region ON jade.product_taxonomy(region_id);
CREATE INDEX idx_product_taxonomy_professional_level ON jade.product_taxonomy(professional_level);

-- GIN indexes for array fields
CREATE INDEX idx_product_taxonomy_functions ON jade.product_taxonomy USING GIN(primary_function_ids);
CREATE INDEX idx_product_taxonomy_concerns ON jade.product_taxonomy USING GIN(skin_concern_ids);
CREATE INDEX idx_product_taxonomy_targets ON jade.product_taxonomy USING GIN(target_area_ids);
...
-- Verification query
SELECT 'Taxonomy schema created successfully!' AS status;
SELECT COUNT(*) as category_count FROM jade.product_category;
SELECT COUNT(*) as function_count FROM jade.product_function;
SELECT COUNT(*) as concern_count FROM jade.skin_concern;
